"""
SafeKhao Backend — Flask + MongoDB Atlas + Groq AI
...
# Use a default MongoDB URI (you can change it here or in Render environment variables)
# MONGODB_URI is the standard standard environment variable for MongoDB
DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    os.environ.get("MONGODB_URI", "mongodb+srv://sabatsambit49_db_user:RG9yd1386iRzaPx8@cluster0.tljkviw.mongodb.net/?appName=Cluster0")
)
...
def get_db():
    ...
    return client.get_database("safekhao")
...
def init_db():
    ...
    # We create indexes to ensure barcode uniqueness and fast lookups.
...
def _seed(db):
    if db.products.count_documents({}) == 0:
        ...
        if docs:
            ...
            print(f"  Seeded {len(docs)} new products into MongoDB.")
...
def get_products():
    ...
    return jsonify([_row_to_dict(p) for p in products])
...
def get_product(barcode):
    ...
    if product:
        return jsonify(_row_to_dict(product))
...
def save_product():
    ...
    update_data = {
        "name": data["name"],
        "brand": data["brand"],
        "category": data.get("category", data.get("cat", "Other")),
        "icon": data.get("icon", "📦"),
        "risk": data["risk"],
        "score": data["score"],
...
def delete_product(barcode):
    db = get_db()
    db.products.delete_one({"barcode": barcode})
    return jsonify({"ok": True})
...
def get_scans():
    db = get_db()
    scans_cursor = db.scans.find({}).sort("scanned_at", -1).limit(100)
    
    results = []
    for s in scans_cursor:
        s["_id"] = str(s["_id"]) # stringify ObjectId
        if "scanned_at" in s and isinstance(s["scanned_at"], datetime):
...
def get_stats():
    ...
    ai = db.products.count_documents({"ai_generated": 1})
    
    cats_agg = list(db.products.aggregate([
        {"$group": {"_id": "$category", "cnt": {"$sum": 1}}},
        {"$sort": {"cnt": -1}}
    ]))
    cats = [{"category": c["_id"], "cnt": c["cnt"]} for c in cats_agg if c["_id"]]
    
    risks_agg = list(db.products.aggregate([
        {"$group": {"_id": "$risk", "cnt": {"$sum": 1}}}
    ]))
    risks = [{"risk": r["_id"], "cnt": r["cnt"]} for r in risks_agg if r["_id"]]
...
def get_product_image(barcode):
    ...
    if not img:
        ...
        if img:
            db.products.update_one({"barcode": barcode}, {"$set": {"image_url": img}})
...
ALTERNATIVES = {
    "8901491502238": ["8906050000262"], "8901491400312": ["8906050000262"],
    "8901491502214": ["8906050000262"], "8901012100028": ["8906050000262"],
    "8901012100202": ["8906050000262"], "8901491400008": ["8906050000262"],
    "8901491435000": ["8906050000262"], "8901058852899": ["8901058005851","8901058852912"],
    "8901012308318": ["8901058005851"], "8906001500308": ["8901058005851"],
    "8901012345672": ["8901012310311","8901063070509"], "8901063032712": ["8901063070509","8906009077017"],
    "8901063139329": ["8901063070509"], "8901719840845": ["8901719103025"],
    "8901063032705": ["8901063070509"], "7622201757236": ["8901063070509","8906009077017"],
    "8901012100202": ["8901725116071"], "8902080100071": ["8901058000528","8901399105012"],
    "8902080104079": ["8901058000528"], "8901764032912": ["8901058000528"],
    "8901764013034": ["8901058000528"], "5000112629729": ["8901058000528"],
    "5000112111064": ["8901207010835"], "7622210100511": ["8901058004595","8906022501001"],
    "8901030817182": ["8901030104923"], "8000500254950": ["7622201699130"],
    "8901071706834": ["8901207002281"], "7622200094943": ["7622201699130"],
    "8901058004588": ["8901030869136"], "8901030831690": ["8901207002281"],
    "8906001500049": ["8901234790014","8901234760017"], "8901030898457": ["8901030850523"],
    "8901030937132": ["8901030850523"], "8901030976063": ["8901030869136"],
    "5000008100103": ["8906022501001"], "8901234500606": ["8906022500974"],
    "8901719840845": ["8901207002281"]
}
...
def get_alternatives(barcode):
    ...
    if not alt_barcodes:
        ...
        if product:
            ...
            alts_cursor = db.products.find({
                "category": cat,
                "risk": {"$in": ["ok", "low"]},
                "barcode": {"$ne": barcode}
            }).sort("score", 1).limit(3)
            return jsonify([_row_to_dict(a) for a in alts_cursor])
        return jsonify([])

    results = []
    for bc in alt_barcodes:
        row = db.products.find_one({"barcode": bc})
        if row:
...
def _row_to_dict(d):
    """Clean MongoDB document for JSON response."""
    if "_id" in d:
        del d["_id"]
...
if __name__ == "__main__":
    ...
    try:
        init_db()
        print("  ✓  MongoDB connected & synced")
    except Exception as e:
        print(f"  ✗  DB connection failed (Check MONGODB_URI): {e}")
